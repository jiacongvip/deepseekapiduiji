...
class DoubaoAutomator:
    ...
    async def capture_request(self, request):
        """捕获请求信息"""
        print(f"Request: {request.url}") # Debug
        # 只捕获API相关的请求
        if '/chat/completion' in request.url and 'doubao.com' in request.url:
            print("Captured /chat/completion request!")
            try:
                print(f"Post Data: {request.post_data}")
            except Exception as e:
                print(f"Could not get post data: {e}")
    ...
    async def run_automation(self, message_text="你好，请介绍一下自己", manual=False):
        """运行自动化流程"""
        self.captured = False
        async with async_playwright() as p:
            # 启动浏览器
            browser = await p.chromium.launch(
                headless=not manual,  # 设置为True可以无头模式运行
            ...
            try:
                ...
                # 等待页面加载完成
                print("等待页面加载...")
                # await page.wait_for_load_state('networkidle') # networkidle might hang
                
                print("等待输入框...")
                if not manual:
                    input_element = await page.wait_for_selector('textarea[data-testid="chat_input_input"]', timeout=10000)
                else:
                    print("手动模式: 请在浏览器中登录并发送一条消息...")
                    # 手动模式下，input_element 可能不需要立即获取，或者我们等待更长时间
                    input_element = True 
                ...
                if input_element:
                    ...
                    # 等待请求被捕获或超时
                    wait_time, max_wait_time = 0, 30 if not manual else 300 # 手动模式等待5分钟
