  ...
  const parser = createParser((event) => {
    try {
      ...
      if (result.status != "finish" && result.status != "intervene") {
        const text = result.parts.reduce((str, part) => {
          const { status, content, meta_data } = part;
          if (meta_data && meta_data.tool_result_extra && _.isArray(meta_data.tool_result_extra.search_results)) {
            searchResults = meta_data.tool_result_extra.search_results;
          }
          if (!_.isArray(content)) return str;
          const partText = content.reduce((innerStr, value) => {
            const {
        ...
        if (searchResults.length > 0) {
          const refText = "\n\n**引用来源:**\n" + searchResults.map((ref, index) => `${index + 1}. [${ref.title}](${ref.url})`).join("\n");
          const data = `data: ${JSON.stringify({
            id: result.conversation_id,
            model: MODEL_NAME,
            object: "chat.completion.chunk",
            choices: [
              { index: 0, delta: { content: refText }, finish_reason: null },
            ],
            created,
          })}\n\n`;
          !transStream.closed && transStream.write(data);
