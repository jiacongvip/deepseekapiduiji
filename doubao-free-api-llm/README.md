...
仅使用 `sessionid` 有概率触发风控（例如提示“当前服务访问频繁”）。建议把浏览器里的**完整 Cookie**写入项目根目录的 `session-cookies-auto.json`（或 `session-cookies.json`），服务会自动读取并用于请求。
...
{
  "你的sessionid": {
    "cookie": "你的完整Cookie字符串（建议包含 sessionid/msToken/s_v_web_id 等字段）",
    "device_id": "可选（有更高成功率）",
    "tea_uuid": "可选（有更高成功率）",
    "web_id": "可选（有更高成功率）",
    "room_id": "可选",
...
请求仍然使用 `Authorization: Bearer 你的sessionid` 即可。
